#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Keel 零配置启动器 (POSIX / Linux / macOS)。
# 用法:  ./scripts/keel.sh            # 双击/无参数 -> 打开网页界面
#        ./scripts/keel.sh --help
#        ./scripts/keel.sh audit out/xxx/ir.json
#
...
# 切到项目根目录，这样 `python -m keel` 才能找到 keel 包。
cd "$ROOT" || { echo "Keel: 无法进入项目目录 $ROOT" >&2; exit 2; }
...
if [ -x "$VENV_PY" ]; then
    PY="$VENV_PY"
# 2) 否则找一个已装好 keel + pydantic 的系统 python。
elif PYBIN="$(command -v python3 || command -v python)"; then
    if "$PYBIN" -c "import keel, pydantic" >/dev/null 2>&1; then
        PY="$PYBIN"
    fi
fi
...
if [ -z "$PY" ]; then
    echo ""
    echo "Keel: 没有可用的 Python 环境，正在尝试创建本地 .venv 并安装依赖..."
    echo ""

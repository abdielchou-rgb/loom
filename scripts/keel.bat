...
rem ---------------------------------------------------------------------------
rem Keel 零配置启动器 (Windows)。
rem 双击 scripts\keel.bat 即可使用，无需懂 python / venv。
...
rem 切到项目根目录，这样 `python -m keel` 才能找到 keel 包。
cd /d "%ROOT%" || (
    echo Keel: 无法进入项目目录 "%ROOT%" >&2
    exit /b 2
)
...
rem 2) 否则找一个已装好 keel + pydantic 的系统 python。
where python3 >nul 2>nul && set "PY=python3"
if not defined PY (
    where python >nul 2>nul && set "PY=python"
)
if defined PY (
    "%PY%" -c "import keel, pydantic" >nul 2>nul
    if not errorlevel 1 (
        goto :launch
    )
)

...
之内的重复）。两条外部依据：
  ...
  2. Genette, G. (1972) *Discours du récit* —— 叙事频率 (frequency)。
     ...
     的窗口化重复 (windowed repetition)。Keel 现有校验器全是整篇统计，缺
...
# 分类学与 `check_saturation` 已上移到 `keel/ir/devices.py`（见该文件说明）。
# 此处重新导出，`from keel.runtime.cooldown import ALL_PATTERNS` 仍然可用 ——
# 单一真源只有一处，这里只是转发。
__all__ = [
    "ALL_PATTERNS",
    "CONFLICT_TYPES",
...
class EventCooldownMatrix:
    ...
    def check_saturation(
        ...
        """窗口化重复检测。**实现已上移到 `keel/ir/devices.py`**，此处仅转发。

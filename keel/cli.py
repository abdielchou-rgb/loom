"""Keel 命令行入口。
    ...
    python -m keel.cli write    "一句话想法" --out out/       想法 -> IR -> 正文
    python -m keel.cli web      [--port 8000]              启动本地网页界面（浏览器里操作）
    python -m keel.cli audit    <ir.json>     跑故事体检
    python -m keel.cli proposals <ir.json>    列出结构提案（待裁决）
    python -m keel.cli decide   <ir.json> --all --accept   一键采纳（唯一的生效入口）
    python -m keel.cli audience <ir.json>     观众模拟（留存曲线 + 原话）
    python -m keel.cli render   <ir.json> -f text|fountain|storyboard|ink|renpy
    python -m keel.cli outline  <ir.json>     输出大纲
    python -m keel.cli lore     <ir.json> --text "沈砚掏出腰牌"
    python -m keel.cli play     <ir.json> --runs 500
    python -m keel.cli compliance <ir.json>
    python -m keel.cli submit-check <ir.json>    投稿前合规自检（能不能投）
    python -m keel.cli process-report <ir.json> --out 过程.md   创作过程报告（申诉举证）
    python -m keel.cli templates
    python -m keel.cli api handshake           插件契约：稳定 JSON（薄客户端用）
"""
...
def _load(path: str) -> NarrativeIR:
    ...
    读取本身只有一份实现（`keel/ir/loading.py`）；这里只负责**面向人**的表达。
    面向插件的表达在 `keel/api.py`（结构化错误），两者故意不同：

    为什么不让它直接抛 `FileNotFoundError`：那条 traceback 对作者
    一点用都没有 —— 它说的是「Python 打不开文件」，而作者需要知道的
    是「那份 IR 不在这儿，先跑 keel write 生成一个」。
    ...
    try:
        ...
        if exc.kind == IRLoadError.MISSING:
            raise SystemExit(
                ...
                f"  先生成一个：keel write \"一句话想法\" --out out/\n"
                f"  再来看它：  keel audit out/<目录>/ir.json\n"
            ) from exc
        # 坏 IR 也要说人话（JSON 截断 / 版本不对 / 字段非法）
        raise SystemExit(
            f"\n  读不出这份 IR：{exc.path}\n  原因：{exc.reason}\n"
            f"  若是旧版本生成的，用 keel write 重跑一份即可。\n"
        ) from exc
...
def write_story(
    *,
    ...
    """想法 -> IR -> 正文，并写出全部产物（含 keel.html 枢纽）。
    ...
    **这是 CLI `write` 与本地网页服务共用的唯一生成入口** ——
    ...
    # 正文：网文媒介默认**不出**（平台禁 AI 直出正文，见 keel/policy.py）。
    ...
    try:
        ...
        print(f"\n  ⚠ 未生成正文：{exc}")
        print("    需要正文时加 `--force-prose`（风险自负，见 keel/policy.py）。")
...
def cmd_write(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # 否则脚本无法判断该重试还是该报错。
        print(f"\n  ⏸  {exc}")
        print("\n  这是 workbuddy 驱动：Keel 不发请求，它发问题 ——")
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if pending:
        ...
        print(f"      keel proposals {outdir / 'ir.json'}")
        print(f"      keel decide {outdir / 'ir.json'} --all --accept"
              "   # 或去掉 --accept 表示驳回")
        print("    驳回的证据力强于采纳（见 keel process-report）。")
        if res.paused and args.checkpoint:
            ...
            print(f"\n  续跑（带上你的裁决）：")
            print(f"      keel run \"{idea}\" --checkpoint --resume-from {paused_ir}")
...
def cmd_web(args: argparse.Namespace) -> int:
    """启动本地网页界面（零依赖），让非技术用户也能在浏览器里操作 Keel。
...
def cmd_recheck(args: argparse.Namespace) -> int:
    ...
    能让曲线重新上翘的东西。Keel 发布新校验器时，老用户的旧 IR
...
def cmd_submit_check(args: argparse.Namespace) -> int:
    ...
    与 `compliance` 的区别：
        ...
        submit-check  报 **能不能投**（法律义务 + 平台画像 + Keel 检不了的清单）

    退出码：0 = 无硬阻塞；1 = 有硬阻塞（AI 占比越线 / 溯源不可证明）。
    **「待人工确认」不算阻塞** —— 把它算成通过是在宣称 Keel 全查过了，
...
def cmd_proposals(args: argparse.Namespace) -> int:
    ...
    for d in shown:
        ...
        print()
    print("裁决：keel decide <ir.json> --accept <id> | --reject <id> | --all --accept")
...
def cmd_decide(args: argparse.Namespace) -> int:
    ...
    **这是提案生效的唯一入口。** 裁决结果会记入决策遥测
    ...
    # 依据 CHI 2026 Reactive Writing，详见 keel/provenance/awareness.py。
...
def cmd_c2pa(args: argparse.Namespace) -> int:
    ...
    Keel 只生产 manifest，**不签名**（签名需 C2PA 凭证，超出零依赖范围）。
...
def cmd_bench(args: argparse.Namespace) -> int:
    ...
    差异化于 story-bench（其用 50% LLM-judge 判定）：keel-bench 的所有判定
    ...
    out = {
        ...
        "differentiator": (
            "story-bench 用 50% LLM-judge 判定；keel-bench 全程序化、"
            "可复现、可证伪，且不预测作品是否成功。"
        ),
...
def cmd_api(args: argparse.Namespace) -> int:
    ...
    **静默**弄坏插件，而且没有任何测试会红 —— 因为插件不在 Python 测试里。
    ...
    payload = keel_api.call(args.method, params)
    print(
        json.dumps(
            payload,
            ensure_ascii=False,
    ...
    if payload.get("ok"):
        return 0
    return 6 if keel_api.is_internal(payload) else 1


def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="keel", description="Keel 叙事编译器")
    # `--version` 必须挂在顶层：打包成 console script 之后，用户第一件事往往
    # 是确认自己装的是哪个版本。argparse 的 version action 会直接打印并退出，
    # 不会掉进「不是已知子命令」的分支。
    p.add_argument("--version", action="version",
                   version=f"keel {__version__}")
    ...
    w.add_argument(
        ...
        help="驱动方式：mock=离线桩件（形态对、内容假）；"
             "workbuddy=Keel 把提示词写成待填文件，由 WorkBuddy/人填答案"
    ...
    w.add_argument("--force-prose", action="store_true",
                   help="强制生成网文正文（默认按平台合规策略挡住，见 keel/policy.py）")
    ...
    ap.add_argument("method", help="方法名；先跑 `keel api handshake` 看全部签名")
    ...
    wb = sub.add_parser("web", help="启动本地网页界面（浏览器里操作 Keel，零依赖）")
...
def _guided_write() -> int:
    ...
    print("\n   Keel 会生成一版草稿并做结构体检——它指出问题，决定权在你。")
...
def _open_existing() -> int:
    base = Path("out")
    hubs = sorted(base.glob("*/keel.html"))
    if not hubs:
        print("\n   还没有生成过故事（out/ 下没有 keel.html）。先选 [2] 生成一个吧。")
        return 0
    print("\n   已生成的枢纽：")
...
def _interactive_menu() -> int:
    ...
    print()
    print("=" * 56)
    print("  Keel · 叙事编译器（你的对抗性编辑）")
    print("=" * 56)
    print("  Keel 不替你写，也不夸你写得好。")

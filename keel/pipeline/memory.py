"""动态叙事记忆（Dynamic Narrative Memory）—— Keel 目前**完全缺失**的那一层。
...
**去掉记忆比去掉校验更糟 —— 记忆的贡献大于校验。** 而 Keel 只做了其中一半：
`StateDelta` + 结构校验器就是「符号状态转移校验」那一半；「动态叙事记忆」
这一半此前**不存在** —— 没有对象随场景推进而更新、能被后续场景检索。
本模块补的正是这一半，按消融证据排序，它是 Keel 当前最大的单点能力缺口。
...
class NarrativeMemory(KeelModel):
    """随场景推进而更新的叙事状态。

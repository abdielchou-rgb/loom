"""流水线引擎：Idea -> IR 的各个阶段。
    ...
    TensionSeeder    信念层 + 真值层 -> **张力点**（Keel 第一个「向前看」的引擎）
...
# ---------------------------------------------------------------------------
# Stage 4.6：张力播种（Keel 第一个**向前看**的引擎）
# ---------------------------------------------------------------------------
...
class TensionSeeder:
    ...
    Keel 其余引擎要么在生成（Premise / Cast / Structure / Character），
...
class Proposer:
    ...
    详见 `keel/ir/proposal.py` 的模块 docstring。
...
class CriticLoop:
    ...
    def run(self, ir: NarrativeIR) -> tuple[NarrativeIR, list[str]]:
        ...
        if self.propose:
            ...
            # 却感觉完全掌控。详见 keel/provenance/awareness.py。

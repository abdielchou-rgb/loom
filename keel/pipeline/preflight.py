...
**它守的是哪一道门（为什么它值得存在）：**
自主模式（`keel run`）让机器自己一场一场往下写，直到某个由 IR 派生的停止条件
...
**每条判据的出处（无出处即不发货，铁律 3）：**
    ...
    ending_anchor     项目自述的设计原则 + Egri
                      * `keel/llm/prompts.py::PREMISE` system：
    ...
    premise           `keel/llm/prompts.py::PREMISE` system 明文：
                        「前提（premise）必须是一句因果断言，不是主题词。
                        反例：「关于信任」。正例：「偏执地独自承担一切，
                        会把最该信任的人推成敌人。」」+ Egri 同上（前提 = 因果断言）。
    commitment        Riedl & Young, IPOCL（见 `keel/ir/models.py` 模块 docstring）：
...
**刻意不做的事：**
* 不调用 `keel.validators.base.register()` —— 它不是「故事体检报告」的一行，
...
def _check_ending_anchor(ir: NarrativeIR) -> list[Finding]:
    ...
    出处：`keel/llm/prompts.py::PREMISE`（明文要求「具体到动作或画面，
...
#: 主题词形态：`keel/llm/prompts.py` 点名的反例就是「关于信任」。
...
def _check_premise(ir: NarrativeIR) -> list[Finding]:
    ...
    出处：`keel/llm/prompts.py::PREMISE` system 明文 ——
...
def _check_commitment(ir: NarrativeIR) -> list[Finding]:
    ...
    出处：Riedl & Young, IPOCL（见 `keel/ir/models.py`：L3 承诺层是防
...
def _check_controlling_idea(ir: NarrativeIR) -> list[Finding]:
    ...
    `keel/validators/drift.py::thesis_drift` 拿 `controlling_idea` 当基线，
...
def _check_character_need(ir: NarrativeIR) -> list[Finding]:
    ...
    人物弧光的引擎；项目自述同旨，`keel/llm/prompts.py::CHARACTERS` system：

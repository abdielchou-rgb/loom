"""理论心智（Theory of Mind）信念层 —— 与客观真值层比对的张力生成。
    ...
    于是 Keel 结构上无法表达戏剧反讽，而这是最基础的悬念机制之一。

    本模块的两个立场（与 Keel 现有 24 个校验器相反）
    ────────────────────────────────────────────────────────────
    1. **张力点是生成资产，不是违规报告。**
...
class CharacterBeliefState(KeelModel):
...
class TensionPoint(KeelModel):
    """一个可写的张力点。

    **这是生成资产，不是违规报告。** 与 `keel.validators.base.Finding`

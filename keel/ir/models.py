...
# `KeelModel` 定义在 base.py（见该文件说明为什么必须下沉）。
# 此处重新导出，`from .models import KeelModel` 仍然可用。
#
# ⚠️ 本模块**不能**定义 `__all__`：`keel/ir/__init__.py` 用的是
...
class CharacterState(KeelModel):
    """角色在某个时间点的状态快照。"""
...
class CharacterLayer(KeelModel):
...
class CommitmentLayer(KeelModel):
...
class Relation(KeelModel):
    """关系 —— ⑫ 有有效期的时序边。

    关系不是永久事实，是有起止的边。Keel 原先只有
...
class RecursionPolicy(KeelModel):
...
class Condition(KeelModel):
    quality: str
    op: Literal["==", "!=", ">", ">=", "<", "<=", "in", "not_in"]
    value: Any


class QualityEffect(KeelModel):
    quality: str
    op: Literal["set", "add", "sub"]
    value: Any


class Choice(KeelModel):
    id: str
    text: str
...
class NarrativeIR(KeelModel):
    ...
    # `NarrativeMemory` 住在 `keel/pipeline/memory.py`，而本模块被它依赖，

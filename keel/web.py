...
把「普通人也能操作 Keel」做成现实：一个本地 `http.server`，首屏只有一张
表单（一句话想法 + 几个选择），点「生成」即跑与 CLI `write` **完全相同**的
流水线，产物汇成 `keel.html` 枢纽，直接在浏览器里打开。
...
def build_form_page(
    *,
    ...
    preview_notice = (
        ...
        "要真正使用：在终端运行 <code>keel web</code>"
        "（或 <code>.venv/Scripts/python.exe -m keel.cli web</code>），"
        "再打开它给出的地址。</div>"
        if preview else ""
    )
    go_cls = "go disabled" if preview else "go"
    go_dis = " disabled" if preview else ""
    go_text = "预览模式 · 请在终端启动 keel web" if preview else "生成并体检"
...
{generator_opts}
        ...
        <p class="hint">「WorkBuddy 驱动」不需要 API key：Keel 把提示词写成待填文件，
...
def build_pending_page(
    *,
    ...
    """「Keel 在等你」那一页：逐条列出待填请求。

    为什么单独一页而不是塞进表单：这是 workbuddy 驱动的**主界面**，
    不是错误页。它要能让人（或让一个聊天窗口）看清楚「Keel 到底问了什么」，
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Keel · 等你填 {len(asks)} 个请求</title>
<style>
  :root {{
    --bg:#f5f5f7; --ink:#1d1d1f; --mut:#6e6e73; --line:#d2d2d7;
    --acc:#0071e3; --acc-h:#0077ed; --err:#d70015;
...
<body>
<div class="wrap">
  <h1>Keel 在等你：{len(asks)} 个请求</h1>
  <p class="sub">Keel 不发请求，它发问题。下面每条都是它渲染好的提示词 ——
...
def _make_handler(writer, base_out: Path):
    ...
    class KeelHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
        server_version = "KeelLocal/1.0"
        ...
        def _redirect(self, result: dict) -> None:
            ...
            self.send_header(
                "Location", "/out/" + urllib.parse.quote(rel, safe="/") + "/keel.html"
            )
            self.end_headers()

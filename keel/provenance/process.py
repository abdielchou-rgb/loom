...
── 两类证据（这个区分是本模块存在的理由）────────────────────
    ...
    **免责证据（exculpatory）**  「这一段不是 AI 写的」
                                 → Keel 已有：`ProvenanceLedger` 记 AI 占比
...
Keel 目前**没有**卡片级编辑历史、**没有**版本 diff；裁决时刻**有**了
（墙钟，见 `keel/clock.py`），但它是**自报**的 —— 不签名、不存证。
...
GAPS: tuple[str, ...] = (
    "卡片级编辑历史：Keel 不记录「谁在什么时候把哪个字段从 A 改成 B」。"
    "要补这类证据，请自行保留 IR 的版本快照（每次大改导出一份 ir.json）。",
    "正文的逐字修改过程：Keel 只按场景记 origin，不记段内编辑。"
    "深度改写请保留自己的草稿。",
    "时间戳的**可信度**：本报告记录的是**墙钟时刻**（`Diff.proposed_at` / "
    "`Diff.decided_at`，见 `keel/clock.py`），但那是 Keel **自报**的 —— "
    "不签名、不做第三方存证，改系统时间即可改它。"
    "需要对抗性质疑时，请用第三方时间戳服务或可信存证，别只交这一份。",
    "外部工具链（灵感来源、素材、笔记）：不在 Keel 的能力范围内。",
)
...
class CreativeProcessReport:
    ...
    def to_markdown(self) -> str:
        ...
        L = [f"# {self.title}", ""]
        L.append(
            f"> 由 Keel 生成于 {datetime.now().isoformat(timespec='seconds')}。"

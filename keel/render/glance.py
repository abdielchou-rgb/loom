...
process-report 申诉证据）都是「事后 / 深读」场景。新用户第一次打开 Keel，
...
def render_glance(ir: NarrativeIR, report=None, *, title: str | None = None) -> str:
    ...
    parts = [
        ...
        f"<title>{escape(title or ir.title)} · Keel 一屏速览</title>",
        f"<style>{_CSS}</style></head><body><div class='wrap'>",
        f"<h1>{escape(ir.title)}</h1>",
        f"<div class='sub'>Keel 一屏速览 · {escape(str(ir.medium))} · {ir.word_count()} 字</div>",
        "<div class='chips'>",
        ...
        "<div class='foot'>Keel · 叙事编译器 · 一屏速览为<strong>结构快照</strong>，"
        "不是质量评级。你做了 "
        f"判断 {aw['decided']} · 采纳 {aw['accepted']} · 驳回 {aw['rejected']}"
        f" · 待裁决 {aw['pending']}。Keel 只做结构性淘汰，不预测作品会不会成功。</div>",
        "</div></body></html>",
    ]
    return "\n".join(parts)

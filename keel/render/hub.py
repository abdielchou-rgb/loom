...
_ACTIONS: list[tuple[str, str, bool]] = [
    ("keel \"想法\"", "一句话想法 → IR → 正文/剧本，并打开本枢纽", True),
    ("keel glance <ir>", "生成可分享的一屏速览卡", True),
    ("keel c2pa <ir>", "导出 C2PA 内容凭证（合规溯源）", True),
    ("keel audit <ir>", "跑故事体检，列全部结构问题", False),
    ("keel report <ir>", "生成深度 HTML 报告", False),
    ("keel audience <ir>", "观众模拟：留存曲线 + 原话", False),
    ("keel render <ir> -f ink", "渲染为 Ink 互动小说", False),
    ("keel render <ir> -f fountain", "渲染为 Fountain 剧本", False),
    ("keel render <ir> -f renpy", "渲染为 Ren'Py galgame", False),
    ("keel outline <ir>", "输出纯大纲", False),
    ("keel proposals <ir>", "列出结构提案（待裁决）", False),
    ("keel decide <ir> --all", "一键采纳全部提案（唯一生效入口）", False),
    ("keel compliance <ir>", "合规画像", False),
    ("keel submit-check <ir>", "投稿前合规自检（能不能投）", False),
    ("keel process-report <ir>", "创作过程报告（申诉举证）", False),
    ("keel bench <ir>", "导出 keel-bench 校验器基准", False),
]
...
def render_hub(
    ...
    *,
    ...
    parts = [
        ...
        f"<title>{escape(title or ir.title)} · Keel 枢纽</title>",
        f"<style>{_CSS}</style></head><body><div class='wrap'>",
        f"<h1>{escape(ir.title)}</h1>",
        f"<div class='sub'>Keel 一屏枢纽 · {escape(str(ir.medium))} · "
        ...
        "</div>",
        "<div class='foot'>Keel · 叙事编译器 · 一屏枢纽为<strong>导航 + 结构快照</strong>，"
        "不是质量评级。你做了判断 "
        f"{aw['decided']} · 采纳 {aw['accepted']} · 驳回 {aw['rejected']}"
        f" · 待裁决 {aw['pending']}。Keel 只做结构性淘汰，不预测作品会不会成功。"

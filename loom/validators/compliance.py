...
│ 平台改口径时应当只改这一个文件 + `loom/audit/rhythm.py`，            │
│ 而不需要在文学校验器里翻找哪几条是「合规向」的。                     │
└──────────────────────────────────────────────────────────────────────┘

**本文件只有薄包装，没有算法。** 计算全在 `loom/audit/rhythm.py`
（零依赖、可单测、可被 `rhythm.scan_ir` 单独调用）。
这个分工沿用 `numeric_fact_consistency` 包装 `loom/audit/csn.py` 的先例：
...
── 红线（与 `loom/audit/rhythm.py` 一致，此处再写一次是因为这是入口）──

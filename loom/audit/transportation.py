...
── 理论出处 ─────────────────────────────────────────────
  ...
  —— 感官细节是「世界在场」的证据（与 `loom/audit/craft.py` 同源）。
...
问题（Loewenstein 的信息缺口）。Loom 既有的 `anti_slop` / `craft` 只惩罚
...
── 与既有模块的关系（刻意不重复的部分）──────────────────
  ...
  * **不抄源项目的 `open_questions`**（它只是数 `len("？")`）。本模块的
    `scan_ir` 未解问题数取自 Loom 自己的 `Enigma` 状态机（Barthes 阐释
...
def open_questions_at(ir, scene_id: str) -> int:
    ...
    刻意不复用源项目的「数问号」—— Loom 的 `Enigma` 状态机
...
def scan_ir(ir, *, threshold: float = DEFAULT_MIN_OVERALL) -> list[Finding]:
    ...
    真正的结构门禁在 `loom/validators/` 里（与 `craft.py` / `dress.py`

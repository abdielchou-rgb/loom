...
落在了机器的常见区间」，并给出**真的**改写方向；外加一份「哪些事 Loom
证明不了、必须你自己确认」的清单。

**它不是**：反检测。Loom 帮作者「真的写得更好」，不帮作者「看起来像人写的」。
...
── 三层结构（缺一层就是在骗人）──────────────────────────────
  ...
  B. 平台画像   平台公开口径里 Loom 能复现的那些维度。是**统计画像**，不是判决。
  C. 不可检声明 Loom 结构上**看不到**的维度（平台自有语料相似度、困惑度、
                账号行为特征、产物上到底有没有贴标识……）。

只出 A+B 而不出 C，会让作者以为「自检通过 = 不会被判 AI」。
那是一个 Loom 兑现不了的承诺 —— 平台判定用的信息 Loom 永远不全。
**C 层是本模块最重要的部分**，它把「不知道」显式化。

── 维度对照表的状态是**派生的**，不是手写的 ────────────────────

`DIMENSIONS` 里每一行写的只是「这个平台维度对应 Loom 的哪几个检查 code」，
...
class Dimension:
    ...
    `loom` 为空元组 = Loom 结构上不做这个维度（不是「还没做」，是做不了：
    例如平台自有语料相似度，Loom 没有那份语料）。
    """
    ...
    def covered(self) -> bool:
        ...
        能力名的两种形态都要认：
          * registry 里的校验器 code（`loom/validators/`）
          * `loom/audit/` 下**暴露 `scan_ir` 的模块名**（anti_slop / craft /
            dress / transportation / cognitive …）
            它们不在 registry 里（这是既有设计，见 `Proposer` 的说明），
            但它们是 Loom 真实拥有的能力。

        只认第一种会闹笑话：`dress.py` 明明量了风格漂移，对照表里却显示
        「Loom 不做」—— **派生口径取错，比手写清单更容易骗人**，
        因为它看起来是算出来的。所以两个来源都要扫，且都是现取。
        """
        if not self.loom:
            return False
        return set(self.loom) <= (_REGISTRY_CACHE | _AUDIT_CACHE)
...
def _audit_capabilities() -> frozenset[str]:
    """`loom/audit/` 下所有暴露 `scan_ir` 的模块名 —— **扫包派生**，不手写。
...
DIMENSIONS: tuple[Dimension, ...] = (
    Dimension(
        ...
        E_REPORTED,
        "Loom 侧判的是「与声明弧线是否匹配」，与平台的判定口径**不完全相同**",
    ),
    Dimension(
        "foreshadow_cadence",
        "伏笔回收节奏",
    ...
    Dimension(
        ...
        ("dress",),
        E_REPORTED,
        "Loom 用 dress.py 的风格漂移近似；平台的「锚点」定义未公开",
    ),
    Dimension(
    ...
    Dimension(
        ...
        ("length_burstiness",),
        E_REPORTED,
        "**困惑度本身 Loom 不计算**（需要语言模型）；爆发度用分布特征近似",
    ),
    Dimension(
        "watermark_syntax",
...
def required_labels(ir: NarrativeIR) -> list[str]:
    ...
    Loom **提供文案，不能代为确认它真的被贴上去了** —— 那一步发生在
...
def implicit_metadata(ir: NarrativeIR) -> dict:
    ...
    Loom 能做的是**生成这块元数据**；把它写进最终文件是渲染/导出环节的事。
    """
    models = sorted(
        {
            s.model_id
...
WARN = "warn"
BLOCKED = "blocked"
#: Loom 证明不了，必须作者自己确认。**这一态必须存在** ——
#: 把它并进 ok 就是在宣称「Loom 全查过了」。
MANUAL = "manual"
...
class SubmissionChecklist:
    ...
    def verdict(self) -> tuple[str, str]:
        ...
        if self.warnings:
            return (
                ...
                f"{len(self.manual)} 项须作者自行确认（Loom 证明不了）。",
            )
        return (
            "OK",
            f"Loom 可检的部分没有硬阻塞；仍有 {len(self.manual)} 项须作者确认。",
        )
    ...
    def render(self) -> str:
        ...
        L.append("── 平台维度对照 " + "─" * 40)
        L.append(
            f"  Loom 有对应能力的维度 {cov}/{total}。"
            f"**这不等于「平台的判定覆盖了 {cov}/{total}」** —— "
        )
        L.append("  平台的完整维度未公开，且包含 Loom 结构上拿不到的信息。")
        ...
        L.append("")
        L.append("  ⚠ 本自检**不是**「不会被判 AI」的保证。它只报告 Loom")
        L.append("    在确定性维度上看到的东西；平台的判定用了 Loom 看不到的信息。")
        return "\n".join(L)
    ...
    for d in DIMENSIONS:
        ...
        items.append(
            CheckItem(
                ...
                d.key,
                MANUAL,
                f"{d.platform}（Loom 不做）",
                d.note or "不在 Loom 的能力范围内。",
                f"证据等级：{d.evidence}",
            )
        )
    items.append(

"""修复预算与冲突消解 —— 让「自动修订」有天花板、有仲裁、有边界。
    ...
    **验证器与生成器会互锁**，不设代价预算的修复循环会把预算烧在原地踏步上。
    ...
    Loom 现在的处境：
        `CriticLoop`（`loom/pipeline/engines.py`）只有 `max_rounds=2`
    ...
    2. **冲突消解** `detect_conflicts()`：同一个 (卡, 字段) 被两条互斥建议
       ...
       `loom/ir/proposal.py` 已有的 `DiffStatus.CONFLICTED` 语义与状态机，
       不新造状态、不新造终态。
    ...
    * 状态机与终态语义**完全复用** `loom/ir/proposal.py`（`resolve()`）：
    ...
    因此**不 sleep、不依赖真实时钟**。这条纪律与 `loom/clock.py`
    ...
    纯记账：无 I/O、无网络、不 import 任何其它 `loom.pipeline` 模块

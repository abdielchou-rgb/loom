"""Loom 插件契约（Plugin API）—— 薄客户端**唯一**被允许依赖的接口。
...
所以插件需要一个**稳定的、机器可读的**接口。本模块就是那个接口：
    ...
    python -m loom api handshake
    python -m loom api audit     --ir out/ir.json
    python -m loom api proposals --ir out/ir.json
    python -m loom api decide    --ir out/ir.json --all --accept
    python -m loom api render    --ir out/ir.json --format fountain
...
class ApiError(Exception):
    ...
    #: **Loom 自己出错了。** 与上面所有 kind 都不同：这不是用户的问题，
...
def _require_ir(p: dict[str, Any]) -> NarrativeIR:
    ...
    加载本身只有一份实现（`loom/ir/loading.py`）；这里只负责把它翻译成
    **面向插件**的表达。面向人的表达在 `loom/cli.py::_load`（说人话的 SystemExit）。
...
def _h_decide(p: dict[str, Any]) -> dict[str, Any]:
    ...
    语义与 `loom/cli.py::cmd_decide` 完全一致，因为两者调用的是同一个
    `LoomPipeline.decide` —— 盖章（墙钟 + 主体）与落盘都不在这里重写。
    """
    ir = _require_ir(p)
    ...
    pipe = LoomPipeline(build_generator(offline=True))
    # 同一次调用 = 同一次操作 = 同一个时刻。
    wall = wall_iso()

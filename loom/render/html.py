...
外部依据：
  * **F7（门槛决定采用）**：InkOS（10 agent、37 维审计）在中文横评里只排第 8，
    原因是本地部署 + 命令行 + 门槛太高。Loom 比它更远。
  * **F5（a16z 留存重定基到 M3）**：M0–M3 是获客期，靠的是**分享**。
    一个能双击打开、能发给别人的文件，是 Loom 目前唯一现成的获客载体。
...
def render_html(ir: NarrativeIR, report=None, *, title: str | None = None) -> str:
    ...
    parts: list[str] = [
        ...
        f"<title>{escape(title or ir.title)} · Loom 报告</title>",
        f"<style>{_CSS}</style></head><body><div class='wrap'>",
        f"<h1>{escape(ir.title)}</h1>",
        f"<div class='sub'>Loom 叙事编译报告 · {escape(str(ir.medium))} · "
    ...
    if report.crashes:
        parts.append(
            f"<p class='f err'><b>校验器崩溃 {crashed} 个。</b> 这是 Loom 的缺陷，"
    ...
    if pend:
        parts.append(
            ...
            "Loom <b>不会</b>自动应用结构类改动 —— 自动改结构会毁掉作者意图，"
    ...
    parts.append(
        "<div class='foot'>Loom · 叙事编译器 · 本报告为<strong>结构快照</strong>，"
        "不是质量评级。Loom 只做结构性淘汰，不预测作品会不会成功。</div>"
    )

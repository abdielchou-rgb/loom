...
| `commitment_satisfied` | IPOCL：作者承诺硬约束 |
...
3. 故事体检         健康分 32/100 · 错误 3 · 警告 6 · 提示 8
                    ✗ 承诺未兑现 / ✗ 谜题被遗弃 / ✗ 场景无价值转折
...
1. 七阶段流水线  前提 7 承诺 → 卡司 2 人 → 结构 6 场（价值翻转 6/6）
                → 角色 2 人 / 行动元 7 → 记忆层 6 条 → 台账 5 谜题 + 6 事实
                → 张力 3 点（最强「戏剧反讽」0.80）
2. 正文          1,106 字 / 6 场
3. 结构体检      健康分 92/100 · 错误 0 · 警告 1 · 提示 4
                唯一警告 = AI 占比 100%（这是真实信号，不是 bug）
                → 结构提案 1 条（待作者裁决，**不自动应用**）
4. 观众模拟      加权终局留存 81.7%，4 个人格各自给出不同的抱怨
5. A/B 对比      6 场版 vs 10 场版 → 单场存活率全平，终局留存差 10.5pp
                → 结论：差距纯来自篇幅，改动只加了量没加质
...
9. Token 记账    28 次调用 / 18,518 tokens / $0.1331（粗估）
```

> 正文字数（1,106）会随 **IR schema 变化**而变 —— 离线桩件的随机种子取自
> 场景卡的完整 `model_dump()`，所以给 `SceneNode` 加一个字段就会改变所有
> 生成的句子。这是桩件「确定性但 schema 敏感」的性质，不是缺陷；
> 但它意味着**文档里的字数快照必须重新测，不能沿用**。

**第 5 步是观众模拟最有价值的地方**：它没有说「哪个版本更好」，
而是指出「这两个版本在结构上等价，终局留存的差距是篇幅效应」——
一个如果只看绝对分数就一定会误判的结论。

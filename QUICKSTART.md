...
> Loom 不替你写，也不夸你写得好——它是站在你这边的对抗性编辑。

---

## 0. 先说清它是什么、不是什么

Loom 是**叙事编译器**，不是文本生成器。
...
# 确认装好了
loom --version          # → loom 1.0.0
```

**第一条命令：**

```bash
loom "一个外科医生发现自己缝合的伤口会预言未来"
```
...
```bash
loom proposals out/<目录>/ir.json              # 列出待裁决提案
loom decide    out/<目录>/ir.json --all --accept   # 全部采纳
loom decide    out/<目录>/ir.json prop_xxx --accept # 只裁决一条
```
...
# ── 主流程 ──────────────────────────────────────────────
loom write "想法" --scenes 6 --words 600 --out out/demo1   # 想法 → IR → 正文 → 报告
loom write "想法" --medium micro_drama                     # 换媒介
loom write "想法" --arc icarus --template save_the_cat     # 换情感弧线 / 结构模板
loom web --port 8000                                    # 起本地网页界面（浏览器里填想法、点生成）

# ── 对已有 IR 做事 ──────────────────────────────────────
loom audit       out/demo1/ir.json      # 故事体检
loom recheck     out/demo1/ir.json      # 重跑旧 IR + "上次以来变了什么"
loom audience    out/demo1/ir.json      # 观众模拟（留存曲线 + 观众原话）
loom outline     out/demo1/ir.json      # 大纲（IR 层人工确认）
loom render      out/demo1/ir.json -f renpy                # 渲染成 Ren'Py
loom render      out/demo1/ir.json -f html -o r.html       # 单独出 HTML 报告
loom play        out/demo1/ir.json --runs 500              # 随机化通关测试

# ── 合规 ────────────────────────────────────────────────
loom compliance    out/demo1/ir.json --json out/c.json     # AI 参与度报告
loom submit-check  out/demo1/ir.json                       # 投稿前自检：能不能投
loom process-report out/demo1/ir.json --out 过程.md        # 创作过程留痕（申诉举证）
...
没有 `-m loom` 也能跑：`$P -m loom.cli write "想法"`（等价，不依赖 console script）。
...
第三条路是**让 Loom 发问，而不是生成**：

```bash
loom write "想法" --generator workbuddy --medium screenplay --scenes 3
# ⏸ 待填 7 个请求 → out/<标题>/.wb_queue/<hash>/answers.json
```

Loom 把**渲染好的提示词**写成 `answers.json`，每条留出空的 `output`。
...
原因：网文平台禁 AI 生成正文，而微短剧只要求**标识**。同一个 Loom，
...
| 现象 | 原因 / 处理 |
|---|---|
| `loom: command not found` | console script 装了但**不在 PATH** 上。用下面任意一种：`.venv/Scripts/python.exe -m loom.cli ...`；或直接全路径 `.venv/Scripts/loom.exe`（Windows）／`.venv/bin/loom`（macOS/Linux）；或 `pip install -e .` 后重开终端 |
...
| 启动时端口被占 | **不用管**：`loom web` 会往后自动找一个空闲端口，并打印实际用的地址（如「已自动改用 8001」）。只有连续 20 个端口都不行才需要 `--port 9000` 手动指定 |

...
| 交付物 | 说明 |
|---|---|
| `交付方案_最终.md` | 最终交付方案。所有计数**从代码现取**（`registry_stats()` / `pkgutil` / `len(METHODS)`），不手抄 |
| `交付方案_最终.html` | 自包含版本：0 外部资源、断网可开 |
| `scripts/render_doc.py` | Markdown → 自包含 HTML。**零依赖**，样式全内联 |
| `tests/test_render_doc.py` | 35 条断言，第 1 组是**安全**断言（转义） |

### 为什么渲染器值得存在

项目的交付主张之一是「报告自包含、断网可开」，但**文档本身**此前只有 `.md` ——
要发给别人看，对方得先装 Markdown 阅读器。这跟「第一分钟路径」是同一个病：
**内容对了，但送不到人手上。**

它放在 `scripts/`（文档工具）而不是 `keel/render/`（那六个是**叙事**渲染器，
管的是故事不是文档）—— 与「加一个 renderer，不改核心」的架构纪律一致。

### 安全断言是本模块唯一的硬判据

生成 HTML 最危险的缺陷是**转义**：漏一次 `html.escape`，
正文里的 `<script>` 就变成真的标签 —— 本地看渲染结果「看起来对」，
分享出去却是一条注入路径。

所以第 1 组 7 条断言全部是转义断言，并做了**变异测试**：
把 `_esc` 换成恒等函数 → **9 条变红**（含全部 7 条转义断言）。
证明这个守卫不是空转。

### 一处「报警的是测试不是实现」

第一版断言写成 `"<code>" not in html` 去查「代码块里的反引号不许再套一层 `<code>`」，
但 `<pre><code>` 是代码块的**外层包装**，它本来就该有。
按项目纪律（报警先判断谁错）**改的是测试**：改成数 `<code>` 的**个数**。

### 交付方案里实测出来的两条

1. **第一分钟路径**：`keel write "<想法>" --out ... ` → **0.504 秒**，
   产出 8 个产物；`report.html` 12,804 字节、**0 外链 0 脚本**（断网可开）。
2. **一个安装上的真实坑**：**Python 3.13 新建的 venv 不含 `setuptools`**，
   因此离线环境下 `pip install -e .` 会以
   `BackendUnavailable: Cannot import 'setuptools.build_meta'` 失败。
   解法：先装 `setuptools wheel`，再 `pip install -e . --no-build-isolation`。
   **这条必须写进文档** —— 写错的安装命令比没有文档更贵。

### 状态修正

P5 从 `3/4` 改为 **`3.5/4`**：插件**契约层**已交付（`api.py`，7 个方法，
7 条错误路径实测），但**薄客户端本体仍未做**。
「契约存在」不等于「客户端存在」，两者不能合并计数。

### 验收

```
scripts/verify.py   通过 87 · 失败 0
测试文件            35 / 35 全绿
两个端到端 demo     exit 0
check_launch.py     ALL PASS
```
